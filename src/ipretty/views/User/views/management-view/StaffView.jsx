...
function StudentView(props) {
    ...
    const { reload, status, departments, data } = props
    ...
    const filters = [
        { id: 1, list: status, fieldFilter: 'status', placeholder: getTranslation('CourseStatus'), widthItem: 180 },
        { id: 2, list: departments, fieldFilter: 'departments', placeholder: getTranslation('Department'), widthItem: 180 }
    ]
    ...
    function getListStudentView(params , sort, fieldName, defautSort) {
        UserService.getListUserFollowRole({ ...params, role: 'employee', paging: true },
            res => {
    ...
    function handleShowData(res , sort, fieldName, defautSort) {
        ...
        users.map((user, index) => {
            ...
            user.department_name = user.department && Object.keys(user.department).length > 0 && user.department.department_name ? user.department.department_name : ''
            return user
        })
        if (sort) {
    ...
    function handleData(nameField, value) {
        ...
        } else if (nameField == 'departments') {
            let ids = getListId(departments, value)
            setParams({ ...params, [nameField]: ids })
        } else {
