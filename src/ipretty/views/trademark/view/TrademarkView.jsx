...
const useStyles = makeStyles(theme => ({
    managementView: {
        ...
        "& .view": {
            ...
            marginTop: 28,
            [theme.breakpoints.down('xs')]: {
                padding: '0px',//fixbug64 loi pading lech tren thiet bị mobile
             },
        },
    },
    styleError: {
...
function TrademarkView(props) {
    ...
    const titlePage = getTranslation('Trademark')
    ...
    const actions = [
        { id: 1, action: redirectAdd, icon: <IconImage srcIcon={Plus} />, noIcon: false, label: getTranslation('Addbrand'), buttonClass: 'button button__edit button--white' },
    ]
    ...
    const columns = useMemo(() => [
        ...
        { name: 'bannerUrl', title: getTranslation('Brandlogo') },
        { name: 'title', title: getTranslation('Brandname'), icon: true ,sortData: handleSort },
    ...
    function getListAllPost(params , sort, fieldName, defautSort) {
        BannerService.getAllPost({ ...params, isBanner: 0, isTrademark: 1 , paging: 1 },
            res => {
    ...
    function handleShowData(res, sort, fieldName, defautSort) {
        const trademarks = res.data.data.data
        const total = res.data.data.total
        const current_page = res.data.data.current_page;
        trademarks.map((data, index) => {
            data.stt = index + 1
        ...
        if (sort) {
            trademarks.sort((firtItem, lastItem) => sortFieldName(firtItem, lastItem, fieldName, defautSort))
        }
        setDataTrademark(trademarks)
        setLoading(false)
    ...
    function redirectDetail(value , row) {
        history.push(`/trademarks/${row.post_id}/edit`)
    }

    function redirectAdd() {
        history.push('/trademarks/add')
    }
